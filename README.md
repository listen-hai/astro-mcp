...
| `partOfFortune` | `"reverse_at_night"` (Asc + Moon − Sun by day, Asc + Sun − Moon by night) | `"never_reverse"` (day formula always) | Identical on every day chart; on a night one they can differ by tens of degrees. `partOfFortune.dayChart` says which kind this is. |
